...
BACKUP_EE_DIR_PATH="${LOCAL_BACKUPS_PATH}${LOCAL_DB_NAME}/${CRAFT_BACKUP_SUBDIR}/"
...
echo "Ensuring backup directory exists at '${BACKUP_EE_DIR_PATH}'"
mkdir -p "${BACKUP_EE_DIR_PATH}"

# Backup the Craft-specific dir files via rsync
for DIR in "${LOCAL_EE_FILE_DIRS[@]}"
do
    rsync -F -L -a -z --progress "${LOCAL_EE_FILES_PATH}${DIR}" "${BACKUP_EE_DIR_PATH}"
    echo "*** Backed up assets from ${LOCAL_EE_FILES_PATH}${DIR}"
done
