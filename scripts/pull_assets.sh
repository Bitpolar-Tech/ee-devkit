...
echo "Ensuring Craft files directory exists at '${LOCAL_EE_FILES_PATH}'"
mkdir -p "${LOCAL_EE_FILES_PATH}"

# Pull down the Craft-specific dir files via rsync
for DIR in "${LOCAL_EE_FILE_DIRS[@]}"
do
    rsync -F -L -a -z -e "ssh -p ${REMOTE_SSH_PORT}" --delete-after --progress "${REMOTE_SSH_LOGIN}:${REMOTE_EE_FILES_PATH}${DIR}" "${LOCAL_EE_FILES_PATH}"
    echo "*** Synced assets from ${REMOTE_EE_FILES_PATH}${DIR}"
done
